...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT new com.homesharing_backend.data.dto.PostTopRateDto(p.id, pi.imageUrl, avg(r.point)," +
            ...
            "WHERE p.status = 1 AND ((p.statusReport IS NULL) OR (p.statusReport = 1)) and ((r.status= 1) or (r.status is null)) " +
    ...
    @Query(value = "SELECT new com.homesharing_backend.data.dto.PostTopRateDto(p.id, pi.imageUrl, avg(r.point)," +
            ...
            "left join BookingDetail bd on p.id = bd.post.id " +
            "left join Rate r on r.bookingDetail.id=bd.id " +
            "WHERE p.id =:postID AND ((p.statusReport IS NULL) OR (p.statusReport = 1)) and ((r.status= 1) or (r.status is null)) " +
            " group by p.id order by avg(r.point) desc")
    ...
    @Query("SELECT new com.homesharing_backend.data.dto.SearchDto(p.id, p.title, pd.address, p.price," +
            ...
            "AND ((p.statusReport IS NULL) OR (p.statusReport = 1)) AND p.status= 1 and ((r.status= 1) or (r.status is null)) " +
            "GROUP BY p.id")
    ...
    @Query("SELECT new com.homesharing_backend.data.dto.SearchDto(p.id, p.title, pd.address, p.price," +
            ...
            "AND ((p.statusReport IS NULL) OR (p.statusReport = 1)) AND p.status= 1 and ((r.status= 1) or (r.status is null))  " +
            "GROUP BY p.id")
    Page<SearchDto> listSearchPostByProvinces(@Param("districtID") List<Long> districtID, PageRequest pageRequest);
    ...
    @Query(value = "SELECT new com.homesharing_backend.data.dto.ViewBookingDto(b.id, bd.id, p.id, p.title, bd.startDate, " +
            "bd.endDate, b.totalMoney, bd.totalPerson, bd.totalService, b.note, b.status) FROM Booking b " +
            "LEFT JOIN BookingDetail bd on b.id = bd.booking.id " +
            "LEFT JOIN Post p on bd.post.id = p.id WHERE p.host.id= :hostID AND b.status= 3 AND (current_date() between bd.startDate AND bd.endDate)")
    ...
    @Query(value = "SELECT new com.homesharing_backend.data.dto.PostTopRateDto(p.id, pi.imageUrl, avg(r.point)," +
            ...
            "left join BookingDetail bd on p.id = bd.post.id " +
            "left join Rate r on r.bookingDetail.id=bd.id " +
            "WHERE p.id =:postID and ((r.status= 1) or (r.status is null))  " +
            " group by p.id order by avg(r.point) desc")
